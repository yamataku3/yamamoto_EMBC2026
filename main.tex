...
\begin{figure}[t]
    ...
    \caption{Overview of continual learning scenarios: (A)~CIL and (B)~B-CIL.}
    \label{fig:fig1}
\end{figure}
...
The first is a naïve fine-tuning strategy, in which the model is incrementally updated on each new task without applying any CL methods, serving as a reference point for catastrophic forgetting.
The second is an offline setting, where the model is trained using data from all tasks simultaneously.
...
\begin{figure*}[t]
    ...
    \caption{Three configurations of CIL and B-CIL scenarios for two datasets: (A)~UCI-HAR and (B)~USC-HAD datasets.}
    \label{fig:scenarios}
\end{figure*}
%
...
Hyperparameter optimization was performed using validation data from Task~1 exclusively.
The final hyperparameters for each classification algorithm are summarized in Table~\ref{tab:hyperparameter}.}
...
This finding aligns with previous research in CIL scenarios~\cite{qiao2024class}, which also reported the superiority of replay-based methods.d
